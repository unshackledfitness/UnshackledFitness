...
public class AuthorizedMemberMiddleware
{
	...
	public async Task InvokeAsync(HttpContext context, KitchenDbContext db)
	{
		...
		string? cookbookIdSetting = await db.MemberMeta
			.Where(x => x.MemberId == member.Id && x.MetaKey == KitchenGlobals.MetaKeys.ActiveCookbookId)
		...
		string? householdIdSetting = await db.MemberMeta
			.Where(x => x.MemberId == member.Id && x.MetaKey == KitchenGlobals.MetaKeys.ActiveHouseholdId)
			.Select(x => x.MetaValue)
			.SingleOrDefaultAsync();
