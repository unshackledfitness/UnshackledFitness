...
public static class MemberExtensions
{
	...
	public static async Task<Member> GetMember(this FitnessDbContext db, MemberEntity memberEntity)
	{
		...
		long cookbookId = await db.MemberMeta.GetLongMeta(memberEntity.Id, FitnessGlobals.MetaKeys.ActiveCookbookId);
		if (cookbookId > 0)
		{
		...
		long householdId = await db.MemberMeta.GetLongMeta(memberEntity.Id, FitnessGlobals.MetaKeys.ActiveHouseholdId);
		if (householdId > 0)
		{
	...
	private static async Task AddSampleData(FitnessDbContext db, long memberId)
	{
		...
		await db.SaveMeta(memberId, FitnessGlobals.MetaKeys.ActiveHouseholdId, household.Id.ToString());
