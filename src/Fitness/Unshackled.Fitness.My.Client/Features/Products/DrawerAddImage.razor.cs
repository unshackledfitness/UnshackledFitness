...
public class DrawerAddImageBase : BaseComponent<Member>
{
	...
	protected async Task HandleInputFileChange(InputFileChangeEventArgs e)
	{
		if (e.FileCount > 0)
		{
			...
			var resizedfile = await file.RequestImageFileAsync(e.File.ContentType, FitnessGlobals.MaxImageWidth, int.MaxValue);
			...
			var uploaded = await Mediator.Send(new UploadImage.Command(ProductSid, content));
			if (uploaded.Success)
			{
