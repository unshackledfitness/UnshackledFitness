...
public class UpdateIngredientTitles
{
	...
	public class Handler : BaseRecipeHandler, IRequestHandler<Command, Unit>
	{
		...
		{
			var list = await localStorage.GetItemAsync<IngredientTitleList>(FitnessGlobals.LocalStorageKeys.IngredientTitles);
			...
			if (list == null || list.DateRetrieved <= DateTime.UtcNow.AddMinutes(-LocalStorage.DefaultCacheDurationMinutes))
			{
				...
				await localStorage.SetItemAsync(FitnessGlobals.LocalStorageKeys.IngredientTitles, list, cancellationToken);
			}
			else
			{
				bool updated = false;
