...
public class AuthorizedMemberMiddleware
{
	...
	public async Task InvokeAsync(HttpContext context, FoodDbContext db)
	{
		...
		string? cookbookIdSetting = await db.MemberMeta
			.Where(x => x.MemberId == member.Id && x.MetaKey == FoodGlobals.MetaKeys.ActiveCookbookId)
		...
		string? householdIdSetting = await db.MemberMeta
			.Where(x => x.MemberId == member.Id && x.MetaKey == FoodGlobals.MetaKeys.ActiveHouseholdId)
			.Select(x => x.MetaValue)
			.SingleOrDefaultAsync();
